    ...
    const contents = await Promise.all(
      blobs.map(async (blob) => {
        try {
          ...
          console.warn('无法读取分段为文本:', err)
          return { text: '[无法解析为文本（可能是二进制文件）]', isTruncated: false }
        }
      })
    )

    console.log('contents', contents)
    chunkContents.value = contents.map((c, i) => ({
      size: blobs[i].size,
      text: c.text,
      isTruncated: c.isTruncated,
    }))
  } catch (err) {
    error.value = '预览过程中发生错误：' + (err as Error).message
  }
